...
func physics_update(delta: float) -> void:
	...
	if player.is_on_floor():
		var decel = player.base_stats.calculated_move_speed * 0.5 if player.base_stats else 100.0
...
func handle_input(controller: PlayerController) -> void:
	# Buffer attack input
	if controller.is_attack_pressed() and player._combo_accepting_buffer:
		player._combo_buffered = true
	
	# Dash cancel
	if controller.is_dash_pressed() and player.can_dash() and player._combo_accepting_buffer:
		_reset_animation_speed()
