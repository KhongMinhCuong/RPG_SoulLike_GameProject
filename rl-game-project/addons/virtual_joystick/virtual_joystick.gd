...
## Joystick jump sensitivity
@export_range(0.0, 1.0, 0.05) var jump_threshold_y: float = 0.6
@export_range(0.0, 1.0, 0.05) var jump_threshold_x: float = 0.5

## Joystick down sensitivity
@export_range(0.0, 1.0, 0.05) var down_threshold_y: float = 0.6
@export_range(0.0, 1.0, 0.05) var down_threshold_x: float = 0.5
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if use_input_actions:
		...
		# Press actions with full strength (1.0) for consistent movement speed
		if output.x < -0.2:  # Dead zone threshold
			Input.action_press(action_left, 1.0)  # Always use 1.0 instead of -output.x
		if output.x > 0.2:  # Dead zone threshold
			Input.action_press(action_right, 1.0)  # Always use 1.0 instead of output.x
		if output.y < -jump_threshold_y and abs(output.x) < jump_threshold_x:
			Input.action_press(action_up, 1.0)  # Always use 1.0 instead of -output.y
		else:
			if Input.is_action_pressed(action_up):
				Input.action_release(action_up)
		# Down (crouch / descend)
		if output.y > down_threshold_y and abs(output.x) < down_threshold_x:
			Input.action_press(action_down, 1.0)  # Always use 1.0 instead of output.y
		else:
